...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Ссылка.Пустая() Тогда
		...
		Если СотрудникВШтате(Объект.Ссылка) И Не РольДоступна("Администратор") Тогда
			Элементы.ДатаПриёма.ТолькоПросмотр = Истина;
			Элементы.Должность.ТолькоПросмотр = Истина;
			Элементы.Подразделение.ТолькоПросмотр = Истина;
			Элементы.Оклад.ТолькоПросмотр = Истина;
			Элементы.Уволен.ТолькоПросмотр = Истина;
			
			Сообщить("Для ручного редактирования всех полей обратитесь к администратору.");
		КонецЕсли;
	КонецЕсли;
